...
function aztecencode.find_optimal_sequence(data)
	...
	for _, c in ipairs(data) do
				...
				if cur_len[x] + azcommon.latch_len[x][y] < cur_len[y] then
					...
					if y == "binary" then
						if x == "punct" or x == "digit" then
							back_to[y] = "upper"
							local new_seq = {}
							for _, val in ipairs(cur_seq[x]) do
								table.insert(new_seq, val)
							end
		...
		if has_val(azcommon.upper_chars, c) then
			table.insert(possible_modes, "upper")
		end
		if has_val(azcommon.lower_chars, c) then
			table.insert(possible_modes, "lower")
		end
		if has_val(azcommon.mixed_chars, c) then
			table.insert(possible_modes, "mixed")
		end
		if has_val(azcommon.punct_chars, c) then
			table.insert(possible_modes, "punct")
		end
		if has_val(azcommon.digit_chars, c) then
			table.insert(possible_modes, "digit")
		end
			...
			for _, y in ipairs(azcommon.modes) do
				if y ~= "binary" and y ~= x then
					if cur_len[y] + azcommon.shift_len[y][x] + azcommon.char_size[x] < next_len[y] then
						...
						local seq = {}
						for _, val in ipairs(cur_seq[y]) do
							table.insert(seq, val)
						end
		...
		if prev_c ~= "" and has_val(azcommon.punct_2_chars, prev_c .. c) then
			for _, x in ipairs(azcommon.modes) do
				...
				if last_mode == "punct" then
					if
						#cur_seq[x] > 0
						and has_val(azcommon.punct_2_chars, cur_seq[x][#cur_seq[x]] .. c)
						and x ~= "mixed"
					then
						if cur_len[x] < next_len[x] then
							next_len[x] = cur_len[x]
							local seq = {}
							for idx = 1, #cur_seq[x] - 1 do
								table.insert(seq, cur_seq[x][idx])
							end
...
function aztecencode.generate_matrix(text, requested_size, requested_compact)
	...
	azcommon.reed_solomon(
		codewords,
		data_cw_count,
		cw_count - data_cw_count,
		2 ^ cw_bits,
		azcommon.polynomials[cw_bits]
	)
