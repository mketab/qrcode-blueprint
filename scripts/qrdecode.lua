...
local function is_function_module(version, col, row, size)
	...
	if col >= size - 7 and row <= 8 then
		return true
	end
	if col <= 8 and row >= size - 7 then
		return true
	end

	-- Timing patterns
	if col == 7 or row == 7 then
		return true
	end

	-- Dark module
	if col == 9 and row == 4 * version + 10 then
		return true
	end
		...
		for x = 1, #ap do
			for y = 1, #ap do
				if not (x == 1 and y == 1 or x == #ap and y == 1 or x == 1 and y == #ap) then
					local ap_col = ap[x] + 1
					local ap_row = ap[y] + 1
					if col >= ap_col - 2 and col <= ap_col + 2 and row >= ap_row - 2 and row <= ap_row + 2 then
						return true
					end
				end
			end
	...
	-- Version information
	if version >= 7 then
		if col >= size - 10 and col <= size - 8 and row >= 1 and row <= 6 then
			return true
		end
		if col >= 1 and col <= 6 and row >= size - 10 and row <= size - 8 then
			return true
		end
	end
	...
	if col == 9 and row >= size - 7 then
		return true
	end
	if row == 9 and col >= size - 7 then
		return true
	end
...
local function decode_bits(data_bits, version)
	...
	while true do
		...
		if not mode or mode == 0 then
			break
		end -- End of message or no more bits
...
local function check_finder_pattern(tab, ox, oy)
	local N = #tab
	if ox < 1 or ox + 6 > N or oy < 1 or oy + 6 > N then
		return false
	end

	-- Outer border (must be 1)
	for dx = 0, 6 do
		if tab[ox + dx][oy] == 0 or tab[ox + dx][oy + 6] == 0 or tab[ox][oy + dx] == 0 or tab[ox + 6][oy + dx] == 0 then
			return false
		end
	end
...
local function extract_format_info(tab)
	...
	for r = N, N - 7, -1 do
		table.insert(p2, tab[9][r])
	end
	-- f8..f14 from top-right row 9
	for c = N - 7, N do
		table.insert(p2, tab[c][9])
	end
		...
		if found_any then
			...
			for S = 1, 40 do
				...
				if N >= 21 and N <= 177 and (N - 17) % 4 == 0 then
					...
					local active_count = (has_tl and 1 or 0)
						+ (has_tr and 1 or 0)
						+ (has_bl and 1 or 0)
						+ (has_br and 1 or 0)
