...
function aztecdecode.decode_matrix(matrix)
	...
	local corrected_mode, success =
		azcommon.correct_errors(cws, total_cw_count - init_cw_count, 16, azcommon.polynomials[4])
	if not success then
	...
	-- 5. Error correction
	local corrected_data, data_success =
		azcommon.correct_errors(cw_list, cw_count - data_cw_count, 2 ^ cw_bits, azcommon.polynomials[cw_bits])
