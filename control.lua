...
local function update_gui_status(frame)
	...
	if not (text_box and text_box.valid) then
		return
	end
...
local function is_valid_blueprint_item(item_name)
	...
	if not proto or proto.hidden then
		return false
	end
	...
	if proto.place_result then
		...
		if
			not ent_proto.has_flag("player-creation")
			or ent_proto.has_flag("not-blueprintable")
			or ent_proto.has_flag("placeable-off-grid")
		then
			return false
		end
...
local function are_items_too_similar(fg_item, bg_item)
	...
	if fg_color and bg_color then
		local dist =
			math.sqrt((fg_color.r - bg_color.r) ^ 2 + (fg_color.g - bg_color.g) ^ 2 + (fg_color.b - bg_color.b) ^ 2)
		if dist < 0.15 then
			return true
		end
...
local function open_qr_gui(player)
	...
	if settings.foreground_item and settings.background_item then
		...
		if
			fg_w ~= bg_w
			or fg_h ~= bg_h
			or are_items_too_similar(settings.foreground_item, settings.background_item)
		then
			settings.background_item = nil
		end
	end
...
local function generate_qr_blueprint(player, settings)
	...
	if fg_item and bg_item then
		...
		if fg_w ~= bg_w or fg_h ~= bg_h then
			player.print({
				"qr-gui.error-size-mismatch",
				string.format("%dx%d", fg_w, fg_h),
				string.format("%dx%d", bg_w, bg_h),
			})
			return false
		end
		if are_items_too_similar(fg_item, bg_item) then
			player.print({
				"qr-gui.error-too-similar",
				prototypes.item[fg_item].localised_name,
				prototypes.item[bg_item].localised_name,
			})
			return false
		end
	end
